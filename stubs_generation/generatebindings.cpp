...
public:
  ...
  bool operator==(const EnumDecl &f) { return (this->name == f.name); }

  int operator<(const EnumDecl &f) { return (this->name < f.name); }

  int operator<(EnumDecl f) { return (this->name < f.name); }

  friend int operator<(const string s, const EnumDecl f);
};
int operator<(const EnumDecl f, string s) { return (f.name < s); }

int operator<(const string s, const EnumDecl f) { return (s < f.name); }
...
public:
  ...
  bool operator==(const TypedefDecl &f) { return (this->name == f.name); }

  int operator<(const TypedefDecl &f) { return (this->name < f.name); }

  int operator<(TypedefDecl f) { return (this->name < f.name); }

  friend int operator<(const string s, const TypedefDecl f);
};
int operator<(const TypedefDecl f, string s) { return (f.name < s); }

int operator<(const string s, const TypedefDecl f) { return (s < f.name); }

int operator<(const TypedefDecl s, const TypedefDecl f) { return (s.name < f.name); }
...
std::vector<string> typedef_structs;
                                     ...
                                     CXClientData client_data)
{
  ...
  case CXCursor_TypedefDecl:
  {
    ...
    auto name = getFromCXString(clang_getCursorSpelling(c));
    typedef_structs.push_back(name);
    break;
  }
...
template <typename T>
void removeDuplicateDecls(vector<T> *decls)
{
  std::set<T> s;
  unsigned size = decls->size();
  for (unsigned i = 0; i < size; ++i)
    s.insert((*decls)[i]);
  decls->assign(s.begin(), s.end());
}
...
bool removeDecls2(vector<string> blacklist, string x)
{
  for (auto e : blacklist)
  {
    ...
    if (x.find(lowerCase(e)) != string::npos)
    {
      return true;
    }
  }
...
bool foo2(FuncDecl s)
{
  for (auto f : s.paramtypes)
  {

    if (removeDecls2(blacklist, f))
    {
      return true;
    }
  }

  if (removeDecls2(blacklist, s.resulttype))
  {
    return true;
  }
                      ...
                      std::ofstream &funcs_out)
{
  ...
  removeDecls(blacklist, &typedefs);
  //  removeDuplicateDecls(&typedefs);
  removeDecls(blacklist, &enums);
  removeDecls(blacklist, &funcs);
  removeDecls(blacklist, &strucs);

  for (auto &s : strucs)
  {
    for (auto e : typedef_structs)
    {
      if (s.name == e)
      {
        s.is_typedef = true;
  ...
  for (auto e : enums)
  {
    ...
    if (e.name.find("unnamed at") != string::npos)
    {
      continue;
    }
  ...
  for (auto s : strucs)
  {
    if (s.name.empty() || s.name == "pari_sp")
    {
      continue;
    }
    if (s.name.find("unnamed at") != string::npos)
    {
      continue;
    }
